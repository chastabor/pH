...
from ph.keys import AGENTS, LLM_FAKE, SESSIONS
from ph.persistence.jsonl import read_session
from ph.session import Session
from ph.testing import MountProfile, block_text, stored_log
...
async def test_a_session_survives_a_restart_and_continues(
    mount: MountProfile, tmp_path: Path
) -> None:
    ctx = await mount(_replies("first answer", "second answer"))
    session = ctx.require(SESSIONS).create("s")
    await ctx.require(AGENTS).create(session, FAKE).prompt("first question")
    ...
    resumed = resumed_ctx.require(SESSIONS).adopt(Session("s", seed=events, header=header))
    await resumed_ctx.require(AGENTS).create(resumed, FAKE).prompt("second question")
...
async def test_a_fork_branches_without_touching_the_parent(mount: MountProfile) -> None:
    ctx = await mount(_replies("answer"))
    parent = ctx.require(SESSIONS).create("parent")
    await ctx.require(AGENTS).create(parent, FAKE).prompt("shared question")

    child = ctx.require(SESSIONS).fork(parent, parent.events[-1].seq, "child")
    await ctx.require(AGENTS).create(child, FAKE).prompt("branch question")

    parent_texts = [block_text(m.content[0]) for m in parent.derive_messages()]
    child_texts = [block_text(m.content[0]) for m in child.derive_messages()]
...
async def test_disposal_unwinds_the_whole_tree(mount: MountProfile) -> None:
    ctx = await mount()
    session = ctx.require(SESSIONS).create("s")
...
async def test_every_message_the_model_saw_is_in_the_log(mount: MountProfile) -> None:
    ctx = await mount(_replies("answer"))
    session = ctx.require(SESSIONS).create("s")
    await ctx.require(AGENTS).create(session, FAKE).prompt("question")

    sent = {
        message.id for request in ctx.require(LLM_FAKE).requests for message in request.messages
    }
