...
async def test_a_log_under_any_filename_is_readable(mount: Any, tmp_path: Path) -> None:
    ...
    The view addresses a *path*, not a `<root>/<id>.jsonl` pair, which is why it
    reads the target itself rather than routing the whole load through the store.
    A log renamed on its way to you still opens; only its **ancestors** are
    addressed by id, because their filenames are the only thing a lineage can
    assume.
    """
    ctx = await mount({"id": "session-persistence", "config": {"root": str(tmp_path / "s")}})
    stored = await _stored(ctx, "renamed")
    sent = tmp_path / "from-a-colleague.log"
    sent.write_bytes(stored.read_bytes())

    session_id, records = load_records(str(sent))

    assert session_id == "renamed", "the id comes from the header, not the filename"
    assert records, "a log under an unexpected name rendered nothing"
...
async def test_a_reference_forked_child_renders_its_inherited_history(
    ...
    inherited = [record for record in records if record.source_seq <= boundary]
    assert inherited, "the parent's rows are missing — only the segment was read"
    assert records[0].source_seq == 0, "the conversation starts at its beginning"
    assert records[-1].source_seq > boundary, "and ends in the child's own work"
...
async def test_a_child_whose_ancestor_is_gone_refuses_rather_than_showing_half(
    ...
    An auditor reading a truncated conversation cannot tell it is truncated, so
