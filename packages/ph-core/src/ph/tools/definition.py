...
class ToolRunContext:
    ...
    early."""
    by: Running
    """Who registered that tool, and where (P6-29).
    ...
    definition is present. Looking it up again per stage would be wrong: a tool that
    unregisters itself during its own `execute` is gone from the rebuilt view by
    `finish`, and `finalize_content` would then run under a fallback rather than as the
    row that wrote it.
    """
...
class ToolDefinition:
    ...
    **Coarser than the truth, and stated so.** `ctx.fs.resolve` passes an absolute
    ...
    def render(self, args: JsonValue, value: object) -> tuple[ContentBlock, ...]:
    ...
    def classify(self, args: object) -> ExecutionMode:
    ...
    def irreversible(self, args: object) -> bool:
...
def define_tool[A: BaseModel](
    ...
    **Generic in `A`, and that is the whole of the checking.** `A` is inferred
    ...
    async def run(raw_args: object, run_ctx: ToolRunContext) -> Any:  # noqa: ANN401
        args: Any = raw_args
...
def _default_render(value: object) -> str:
