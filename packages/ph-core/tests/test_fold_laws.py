...
def _turns(log: Any) -> int:
    return sum(1 for event in log.events if event.type == "turn/start")


def _more_turns(previous: int, log: Any, from_seq: int) -> int:
...
def test_a_second_implementation_that_drifts_is_caught_at_the_event_that_breaks_it() -> None:
    ...
    def counts_ends_too(previous: int, log: Any, from_seq: int) -> int:
        return previous + sum(
...
def test_extending_over_nothing_must_change_nothing() -> None:
    def always_one_more(previous: int, log: Any, from_seq: int) -> int:
