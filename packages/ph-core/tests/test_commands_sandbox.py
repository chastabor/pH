...
async def _run(ctx: Any, line: str, session: Any = None) -> str:
    shown = await ctx.commands.dispatch(line, session=session)
    assert isinstance(shown, str)
    return shown


async def test_show_says_what_is_in_force(mount: Any) -> None:
    ctx = await mount()
...
async def test_allow_host_is_live_before_it_is_kept(mount: Any) -> None:
    """Applied, then kept — in that order, and the seam sees it at once."""
    ctx = await mount()
    assert not ctx.sandbox.permits("example.com", 443)

    shown = await _run(ctx, "/sandbox allow host example.com")

    assert "example.com is now reachable" in shown
    assert ctx.sandbox.permits("example.com", 443)
    assert ctx.sandbox.allowances is not None
    assert ctx.sandbox.allowances.network.hosts == [*DEFAULT_HOSTS, "example.com"]
    assert "Not saved" in shown, "a test composition has no profile name to save under"
    assert dict(ctx.mount.topology())["sandbox-allow"].endswith(", reconfigured live")


async def test_the_change_is_kept_as_a_drop_in_under_the_profile(mount: Any) -> None:
    ...
    ctx.mount.profile.name = "headless"
...
async def test_a_host_is_normalised_the_same_way_in_both_directions(mount: Any) -> None:
...
async def test_allow_and_revoke_path(mount: Any, tmp_path: Path) -> None:
    ctx = await mount()
    ...
    assert ctx.sandbox.allowed_paths() == (cache,)

    shown = await _run(ctx, f"/sandbox revoke path {cache}")
    assert "is no longer writable beyond the workspace" in shown
    assert ctx.sandbox.allowed_paths() == ()


async def test_network_mode_is_switched_by_name(mount: Any) -> None:
    ctx = await mount()
    ...
    assert ctx.sandbox.permits("anything.example", 22)
...
async def test_refusals_name_what_was_wrong(mount: Any, tmp_path: Path) -> None:
    ctx = await mount()
    ...
    assert ctx.sandbox.allowances is not None
    assert ctx.sandbox.allowances.paths == [], "nothing was applied along the way"


async def test_an_unchanged_edit_says_so_and_touches_nothing(mount: Any) -> None:
    ctx = await mount()
    ctx.mount.profile.name = "headless"
    ...
    assert "sandbox-allow" not in ctx.mount.reconfigured


async def test_a_profile_without_the_row_is_told_so(mount: Any) -> None:
    ctx = await mount({"id": "sandbox-allow", "remove": True})
    shown = await _run(ctx, "/sandbox allow host example.com")
    assert "mounts no sandbox-allow row" in shown


async def test_the_footer_counts_refusals_and_show_lists_them(mount: Any) -> None:
    ctx = await mount()
    session = ctx.sessions.create("s")
    agent = ctx.agents.create(session, AgentOptions(provider="fake", model="f"))
    assert ctx.tui_status.readings(session) == [], "nothing refused, nothing said"

    ctx.sandbox.record_denial(
        Denial(kind="network", via="proxy", host="h", port=443), agent=agent.id
    )
    ctx.sandbox.record_denial(Denial(kind="filesystem", via="output", evidence="e"), agent=agent.id)

    assert StatusReading(text="sandbox: 2 refusals", level="warning") in ctx.tui_status.readings(
        session
    )
