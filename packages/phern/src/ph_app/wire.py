...
lister, `phern agents attach` — sees payloads in one of two shapes: **frozen**, when
...
— which a headless `phern agents attach` should never do to render a line of a log it
just received.
...
def result_block(message: object) -> Mapping[str, Any]:
    ...
    `phern agents attach` — skipped it, so `text_of_wire` selected `type: "text"`
