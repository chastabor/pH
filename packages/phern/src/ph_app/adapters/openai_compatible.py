...
async def discover_window(adapter: OpenAiCompatibleAdapter) -> tuple[int, str] | None:
    ...
    **The answering path travels with the answer.** `phern doctor` prints which
...
def _probe_headers(adapter: OpenAiCompatibleAdapter) -> dict[str, str]:
    ...
    `phern doctor` is precisely the command someone runs *before* exporting
...
@dataclass(slots=True)
class _Windows:
    """What `phern doctor` says about each route's window, read when it is asked.
...
async def apply(ctx: Context, config: Config) -> None:
    ...
    if routes.adapters:
        # Every route this row serves, not only the ones that probed: `phern doctor`
