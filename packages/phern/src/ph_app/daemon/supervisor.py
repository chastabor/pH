...
Every mode before this one ties an agent's life to a connection. `phern -p` exits
...
NON_GUARANTEES: tuple[tuple[str, str], ...] = (
    ...
    (
        ...
        "nothing fires. A schedule is kept only while `phern daemon` runs; on start it "
...
Data, not prose: `phern doctor` and `phern agents doctor` print these verbatim, so the
...
class Supervisor:
    ...
    async def rehydrate(self, *, now: int | None = None) -> list[str]:
        ...
        **An index, not a scan, and the reasons are three separate failures.**
        ...
        every session on the machine and the next `phern -p` over any of them is
    ...
    async def sweep(self, *, after: float | None = None, now: int | None = None) -> list[str]:
        ...
        Returns rather than logs so a test — and P5-10's `phern agents` — can ask
