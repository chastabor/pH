"""P6-28 — `phern workspaces gc`, and the count that makes the pile visible.
...
collects what nobody came back for, and a `phern doctor` row that says how many are
...
def test_doctor_counts_what_is_being_kept(tmp_path: Path, roots: Path) -> None:
    ...
    assert "phern workspaces gc" in result.stdout, "the count is only useful beside the way out"
