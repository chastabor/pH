...
def _runtime(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    disagree with `phern agents` about where the daemon is.
...
def test_the_spawned_daemon_is_this_python_and_is_ephemeral() -> None:
    ...
    `sys.executable -m ph_app` because a bare `phern` resolves through `PATH` and may
...
def test_keep_daemon_starts_a_service_instead() -> None:
    ...
    `phern daemon --ephemeral` and `phern --mode tui --keep-daemon` are the two
...
async def test_no_spawn_refuses_and_names_the_command() -> None:
    ...
    For a deployment running `phern daemon` under an init system, where a UI-started
    ...
    assert "phern daemon" in str(raised.value)
