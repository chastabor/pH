"""P5-10 — `phern agents`, the client side of the daemon.
...
async def test_a_follow_ends_when_the_daemon_goes_away(
    ...
    async with serving(tmp_path, monkeypatch) as daemon:
        ...
        async with anyio.create_task_group() as tasks:
            ...
            # Stopped through a second client rather than a second `phern agents
...
async def test_no_daemon_names_the_socket_and_how_to_start_one(
    ...
    for command in (
        ...
        assert "phern daemon" in result.output, command
...
async def test_a_socket_nobody_answers_is_told_apart_from_no_socket(
    ...
    it is *not* the same situation as never having started one: `phern daemon`
    ...
    assert "phern daemon" in result.output
...
async def test_a_reaped_socket_is_not_reported_as_one_never_started(
    ...
    ran `phern daemon` sees. Telling both of them to start one sends the first to a
...
def test_every_agents_command_is_registered() -> None:
    ...
    `phern daemon` once stopped being a registered command because a module-level
