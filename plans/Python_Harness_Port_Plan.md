...
- The transport is **renamed to `ipython`** for the RLM profile (a presentation-level alias for `run_code`; the reserved name is still `run_code` internally). Description ported verbatim from `src/core/tools/ipython.ts`: *"Python scratchpad code or `%%bash` shell cells to execute in the agent kernel. Use the target project's own environment for project imports, tests, scripts, CLIs, and dependency checks instead of direct kernel imports."* `execution_mode: exclusive`.
- Result text to the model = `stdout + "\n" + stderr + "\n" + result + "\n" + traceback` (prime-agent's exact concatenation); images become `image` blocks; `details: IpythonToolDetails {diffs, attachments, sent_agent_messages, duration_ms, status}` drives the TUI cell renderer; `is_error` on `error|aborted`. Partial output streams via `on_update` → `ToolExecutionUpdate`.
...
4. Detached task (an `anyio` task in the agent scope, or a worker in daemon mode): `ctx.subagents.start_continuable("rlm-child", …)` → child `Agent` with the RLM preset, `RLM_DEPTH+1`, inherited provider/model/thinking/skills/tools; the task arrives as a `user/message` with `source: plugin{rlm, form: relay}` and text `[task from parent]\n\n<prompt>`; then wait for quiescence.
